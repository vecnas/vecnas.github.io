jiant.module("jiant-logic", ["jiant-util"], function($, app, jiant, params, Util) {
  ...
  function copyLogic(appId, name) {
    ...
    if (obj && appId in awaitingDepends && name in awaitingDepends[appId] && app) {
      ...
      $.each(typeof obj === "function" ? obj($, app) : obj, function(fname, fspec) {
  ...
  function declare(name, objOrUrlorFn) {
    ...
    function handle() {
      ...
      $.each(awaitingDepends, function(appId) {
...
//            handleBound(appId, cb);
  ...
  function beforeBind(appId, dep, cb) {
    ...
    loadedLogics[appId] = loadedLogics[appId] || {};
    dep && $.each(dep, function(idx, depName) {
  ...
  function isDependResolved(appId, cbObj) {
    ...
    awaitingDepends[appId] && $.each(awaitingDepends[appId], function(depName, cbArr) {
